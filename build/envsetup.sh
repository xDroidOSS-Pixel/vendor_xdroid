...
- githubremote:    Add git remogte for xDroidOSS-Pixel Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/xdroid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/xDroidOSS-Pixel/$PROJECT
